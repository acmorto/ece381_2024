...
const char* ssid = "iPhone-13minus1";
const char* password = "hotspothop";
...
void setup() {
  ...
  ledcAttach(output26, 2000, resolution);  // Set outputs to LOW
        ...
        // that's the end of the client HTTP request, so send a response:
          if (currentLine.length() == 0) {
            ...
            if (header.indexOf("GET /26/on") >= 0) {
              ...
              output26State = "on";
              ledcWriteTone(output26, 290);
            } else if (header.indexOf("GET /26/off") >= 0) {
              Serial.println("GPIO 26 off");
              output26State = "off";
              ledcWriteTone(output26, 0);
            }
